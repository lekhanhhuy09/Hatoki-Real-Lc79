...
// ============================================================
// THUẬT TOÁN 11: PREDICT SUPER V3 (từ Thuật_toán_api.js)
// Dùng pattern memory, dice memory, score memory, biet cau
// ============================================================
// Bộ nhớ nội bộ cho predictSuperV3 (không dùng localStorage)
const _v3Memory = {
    cauBiet: { Tai: {}, Xiu: {}, stats: { maxTai: 0, maxXiu: 0 } },
    patternNext: {},
    scoreAfterResult: {},
    tripleTransition: {},
    movingAvg5: []
...
function _v3UpdateMemory(history) {
    // Cập nhật memory từ history (incremental)
    const n = history.length;
    if (n < 3) return;
    const results = history.map(h => h.tx); // 'T' or 'X'
    const totals = history.map(h => h.total);

    // Cau biet (streak tracking)
    const last = results[n - 1];
    let streak = 1;
    for (let i = n - 2; i >= 0; i--) { if (results[i] === last) streak++; else break; }
    if (streak >= 3) {
        const side = last === 'T' ? 'Tai' : 'Xiu';
        _v3Memory.cauBiet[side][streak] = (_v3Memory.cauBiet[side][streak] || 0) + 1;
        if (streak > _v3Memory.cauBiet.stats['max' + side]) _v3Memory.cauBiet.stats['max' + side] = streak;
    }

    // Pattern next (up to length 8)
    for (let len = 3; len <= 8; len++) {
        if (n > len) {
            const pattern = results.slice(-len - 1, -1).join('');
            const nextKey = pattern + '->' + last;
            _v3Memory.patternNext[nextKey] = (_v3Memory.patternNext[nextKey] || 0) + 1;
        }
    }

    // Score -> result tracking
    if (n >= 2) {
        const prevTotal = totals[n - 2];
        if (!_v3Memory.scoreAfterResult[prevTotal]) _v3Memory.scoreAfterResult[prevTotal] = { T: 0, X: 0 };
        _v3Memory.scoreAfterResult[prevTotal][last]++;
    }

    // Triple transition
    if (n >= 2 && history[n - 1].dice && history[n - 2].dice) {
        const prevTriple = history[n - 2].dice.join('');
        const currTriple = history[n - 1].dice.join('');
        const key = prevTriple + '_to_' + currTriple;
        _v3Memory.tripleTransition[key] = (_v3Memory.tripleTransition[key] || 0) + 1;
    }

    // MA5
    if (n >= 5) {
        const avg5 = totals.slice(-5).reduce((a, b) => a + b, 0) / 5;
        _v3Memory.movingAvg5.push(avg5);
        if (_v3Memory.movingAvg5.length > 200) _v3Memory.movingAvg5.shift();
    }
}

let _v3LastBuilt = 0; // track last history length we built memory for

function algoPredictSuperV3(history) {
    if (history.length < 10) return null;
    const n = history.length;

    // Rebuild memory nếu history thay đổi
    if (n > _v3LastBuilt) {
        const start = Math.max(3, _v3LastBuilt);
        for (let i = start; i <= n; i++) {
            _v3UpdateMemory(history.slice(0, i));
        }
        _v3LastBuilt = n;
    }

    const results = history.map(h => h.tx);
    const totals = history.map(h => h.total);
    const lastTx = results[n - 1];
    ...
    for (let len = 3; len <= 8; len++) {
        if (n >= len) {
            ...
            const nextT = _v3Memory.patternNext[pattern + '->T'] || 0;
            const nextX = _v3Memory.patternNext[pattern + '->X'] || 0;
            const total = nextT + nextX;
            if (total >= 5) {
                const probT = nextT / total;
                predictions.push({ predict: probT > 0.5 ? 'T' : 'X', confidence: Math.abs(probT - 0.5) * 2, weight: 0.02 * len });
            }
        }
    }
    ...
    // 2. Cau biet
    let streak = 1;
    for (let i = n - 2; i >= 0; i--) { if (results[i] === lastTx) streak++; else break; }
    if (streak >= 3) {
        const side = lastTx === 'T' ? 'Tai' : 'Xiu';
        const maxStreak = _v3Memory.cauBiet.stats['max' + side] || 50;
        let countLonger = 0, countThis = 0;
        for (let s = streak + 1; s <= Math.min(50, maxStreak); s++) {
            countLonger += _v3Memory.cauBiet[side][s] || 0;
        }
        countThis = _v3Memory.cauBiet[side][streak] || 0;
        const total = countThis + countLonger;
        if (total > 0) {
            const probContinue = countLonger / total;
            const pred = probContinue > 0.5 ? lastTx : (lastTx === 'T' ? 'X' : 'T');
            predictions.push({ predict: pred, confidence: Math.abs(probContinue - 0.5) * 2 + 0.3, weight: 0.15 });
        }
    }

    // 3. Score-based
    if (_v3Memory.scoreAfterResult[lastTotal]) {
        const sr = _v3Memory.scoreAfterResult[lastTotal];
        const total = sr.T + sr.X;
        if (total >= 5) {
            const probT = sr.T / total;
            predictions.push({ predict: probT > 0.5 ? 'T' : 'X', confidence: Math.abs(probT - 0.5) + 0.3, weight: 0.1 });
        }
    }

    // 4. Be cau khi streak qua dai
    if (streak >= 7) {
        predictions.push({ predict: lastTx === 'T' ? 'X' : 'T', confidence: 0.7 + Math.min(0.2, (streak - 7) * 0.03), weight: 0.12 });
    }

    // 5. MA5 mean reversion
    if (_v3Memory.movingAvg5.length >= 2) {
        const lastMA5 = _v3Memory.movingAvg5[_v3Memory.movingAvg5.length - 1];
        if (lastMA5 > 13) predictions.push({ predict: 'X', confidence: 0.6, weight: 0.05 });
        else if (lastMA5 < 7) predictions.push({ predict: 'T', confidence: 0.6, weight: 0.05 });
    }
    ...
    for (const p of predictions) {
        ...
        if (p.predict === 'T') weightedT += w;
        else weightedX += w;
        totalWeight += w;
    }
    if (totalWeight === 0) return null;

    const probTai = weightedT / totalWeight;
    if (Math.abs(probTai - 0.5) < 0.04) return null; // tín hiệu quá yếu
    return probTai > 0.5 ? 'T' : 'X';
}

// ============================================================
// THUẬT TOÁN 12: AI HTDD + SMART BRIDGE BREAK (từ predictionAlgorithmsAll.js)
// ============================================================
function _htddDetectStreak(history) {
    if (!history.length) return { streak: 0, currentTx: null };
    const last = history[history.length - 1].tx;
    let streak = 1;
    for (let i = history.length - 2; i >= 0; i--) {
        if (history[i].tx === last) streak++; else break;
    }
    return { streak, currentTx: last };
}

function algoAiHtddBridge(history) {
    if (history.length < 5) return null;
    const { streak, currentTx } = _htddDetectStreak(history);
    const tx = history.map(h => h.tx);
    const totals = history.map(h => h.total);
    const n = history.length;

    // --- AI HTDD Logic ---
    // Phát hiện mẫu 1T1X (alternating)
    if (n >= 5) {
    ...
    // Chuỗi dài -> đảo chiều
    if (streak >= 7) return currentTx === 'T' ? 'X' : 'T';

    // Phân tích điểm trung bình
    const recent7Totals = totals.slice(-7);
    const avgScore = recent7Totals.reduce((a, b) => a + b, 0) / (recent7Totals.length || 1);
    if (avgScore > 13) return 'X'; // Điểm cao -> Xỉu
    if (avgScore < 7) return 'T';  // Điểm thấp -> Tài

    // --- Smart Bridge Break Logic ---
    ...
    const xCount = last20.filter(r => r === 'X').length;
    const imbalance = Math.abs(tCount - xCount) / last20.length;
    ...
    // Last 30 score deviation
    const lastScores = totals.slice(-20);
    const avgS = lastScores.reduce((a, b) => a + b, 0) / (lastScores.length || 1);
    const scoreDeviation = lastScores.reduce((a, b) => a + Math.abs(b - avgS), 0) / (lastScores.length || 1);

    if (streak >= 7) breakProb = Math.min(breakProb + 0.15, 0.9);
    else if (streak >= 4 && scoreDeviation > 3.5) breakProb = Math.min(breakProb + 0.1, 0.85);
    else breakProb = Math.max(breakProb - 0.15, 0.15);

    // Quyết định: bẻ cầu hay theo cầu
    if (breakProb > 0.55) return currentTx === 'T' ? 'X' : 'T';

    // Cân bằng tổng thể
    const overallT = tx.filter(r => r === 'T').length;
    const overallX = tx.filter(r => r === 'X').length;
    if (Math.abs(overallT - overallX) / n > 0.3) {
        return overallT > overallX ? 'X' : 'T';
    }

    // Theo chiều gần đây
    const recentT = tx.slice(-7).filter(r => r === 'T').length;
    const recentX = tx.slice(-7).filter(r => r === 'X').length;
    return recentT > recentX ? 'X' : 'T';
}
...
const ALL_ALGS = [
    ...
    { id: 'k_predict_super_v3', fn: algoPredictSuperV3 },
    { id: 'l_ai_htdd_bridge', fn: algoAiHtddBridge }
];
